...
-	<a href="https://github.com/terryyin/hfcca"><b>Header-free cyclomatic
	complexity analyzer</b></a> is a Python script that measures cyclomatic
	complexity and counts the number of lines of code in cpp files, not
	including comments or tests.  If Python is installed and in the path, HFCCA
	can be applied to Om by entering the following at the terminal from inside
	the Om directory:
