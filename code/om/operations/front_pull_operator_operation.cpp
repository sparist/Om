...
// MARK: Om::Operations::FrontPullOperatorOperation
	...
	#define Type_ Om::Operations::FrontPullOperatorOperation

// MARK: public (static)

inline char const * Type_::GetName()
{
	return( Om_Operations_FrontPullOperatorOperation_GetName_() );
}
...
inline void Type_::Give( Evaluation & theEvaluation )
{
	theEvaluation.TakeOperation(
		std::auto_ptr< Operation >(
			new PullOperation< Literal, FrontPullOperatorOperation >
		)
	);
}
		...
		SUITE( FrontPullOperatorOperation )
		{
			TEST( Definition )
			{
				CHECK_EQUAL(
					"{operator<-elements}",
					System::Get().Evaluate(
						"drop find {operator<-elements} system"
					)
				);
			}
