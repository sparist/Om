...
// MARK: Om::DefaultElement
	...
	#define Type_ Om::DefaultElement< ThisImplementation, ThisInterface >
...
{
	if( typeid( theElement ) == typeid( ThisImplementation ) ){
		assert( dynamic_cast< ThisImplementation const * >( &theElement ) );
	...
	return( this->IsEmpty() && theElement.IsEmpty() );
}
...
inline void Type_::GiveElements( Queue & theQueue )
{
	...
	if( !this->IsEmpty() ){
		theQueue.TakeElement( static_cast< ThisImplementation & >( *this ) );
	}
}
